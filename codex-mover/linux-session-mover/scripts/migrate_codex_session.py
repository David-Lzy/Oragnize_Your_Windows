...
def copy_and_validate_rollout(
    ...
    if lines == 0 or meta_records != 1:
        raise MigrationError(
            f"expected exactly one session_meta record; found {meta_records}"
        )
    if destination_cwd is None and source_hash.digest() != target_hash.digest():
    ...
    return RolloutStats(
        ...
        rewritten_cwd_records=rewritten,
...
def verify_target(plan: MigrationPlan) -> None:
    ...
    read_first_session_meta(plan.target_rollout, plan.thread_id)
    ...
    with closing(connect_read_only(plan.target_database)) as connection:
        ...
        row = connection.execute(
            "SELECT rollout_path, cwd FROM threads WHERE id = ?", (plan.thread_id,)
        ).fetchone()
        if row is None:
