        ...
        records = [
            ...
            },
            {
                ...
                "payload": {
                    "role": "user",
                    "content": (
                        "Keep this historical text unchanged: "
                        f"{self.old_cwd}; provider=openai"
                    ),
                },
            },
            {
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with sqlite3.connect(fixture.target_database) as connection:
                ...
                row = connection.execute(
                    """
                    SELECT rollout_path, cwd, model_provider, preview,
            ...
            self.assertEqual(records[3]["payload"]["cwd"], str(fixture.new_cwd))
            self.assertEqual(records[3]["payload"]["model_provider"], "OpenAI")
            self.assertIn(str(fixture.old_cwd), records[1]["payload"]["content"])
            self.assertIn("provider=openai", records[1]["payload"]["content"])
            ...
            self.assertEqual(manifest["destination_model_provider"], "OpenAI")
            self.assertEqual(manifest["rollout"]["session_meta_records"], 2)
            self.assertEqual(manifest["rollout"]["rewritten_cwd_records"], 3)
            self.assertEqual(manifest["rollout"]["rewritten_model_provider_records"], 2)
            self.assertIn(
                "source_only_metadata", manifest["source_only_sqlite_columns"]
            )

    def test_model_provider_is_preserved_when_option_is_omitted(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            fixture = MigrationFixture(Path(temporary))
            command = fixture.command("--apply", "--confirm-codex-stopped")
            provider_index = command.index("--destination-model-provider")
            del command[provider_index : provider_index + 2]
            ...
            records = [
                json.loads(line)
                for line in fixture.target_rollout()
                .read_text(encoding="utf-8")
                .splitlines()
            ]
            session_providers = [
            ...
            self.assertEqual(session_providers, ["openai", "openai"])
