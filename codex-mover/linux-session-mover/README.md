...
> [返回 Codex Mover](../README.md) · [Windows Desktop 兼容](../docs/SSH-REMOTE-SESSION-WINDOWS-COMPATIBILITY.md) · [共享安全指南](../../docs/SAFETY.md) · [测试指南](../../docs/TESTING.md)
...
脚本只迁移 task，不复制 `auth.json`、`config.toml`、API Key、插件或日志。迁移后继续 task 时，实际使用的是**目标 `CODEX_HOME` 当前配置的认证**。如果目标端使用的 provider 内部名称与源端不同，还必须显式传入 `--destination-model-provider`，让 SQLite 与 rollout 元数据一致。
...
2. 找到并逐行验证对应 rollout；兼容大型 task 中重复出现的 `session_meta`。
...
codex_destination_provider="<exact-target-provider-name>"
...
python3 codex-mover/linux-session-mover/scripts/migrate_codex_session.py \
  ...
  --destination-model-provider "$codex_destination_provider" \
...
- destination provider 是否与目标端原生 task 的 `threads.model_provider` 完全一致，包括大小写。
- 目标是否已有同 ID 的 row/file。
- 是否仍检测到使用任一 `CODEX_HOME` 的 Codex 进程。
- `sessions` 与 `archived_sessions` 是否位于同一文件系统。

如果项目没有移动，省略 `--destination-cwd`，脚本会保留原 cwd。如果源、目标 provider 内部名称完全相同，也可省略 `--destination-model-provider`。
...
如果通过 Desktop SSH 连接 Linux，必须让**远程目标 app-server** 重新启动并读取目标数据库。仅重启本地窗口不一定会结束仍由远端进程管理器保留的旧 app-server。远端已经能 `thread/resume`，但原 Windows 点击后仍回到旧 Host 或变成“新任务”时，继续按 [Windows Desktop 路由修复](../docs/SSH-REMOTE-SESSION-WINDOWS-COMPATIBILITY.md) 处理本地状态。
...
- rollout 所有 `session_meta.payload.cwd`。
...
认证文件属于目标 `CODEX_HOME`，但旧 task 的 provider 名称还可能保存在：

- 目标 SQLite 的 `threads.model_provider`。
- rollout 中每一条 `session_meta.payload.model_provider`。

不要假定 provider 名称不区分大小写。先从目标账户原生创建的测试 task 读取实际值：
...
若它与源 task 不同，把精确值传给 `--destination-model-provider`。脚本只改目标副本中的结构化字段，并记录改写数量；不会把历史消息、代码或工具输出中碰巧出现的 provider 文本替换掉。

真实大型 rollout 在压缩或恢复后可以包含多条同 ID 的 `session_meta`。迁移器会逐条验证 ID，并改写**全部** provider/cwd 元数据；不能只改第一条，也不能因记录数大于 1 就判定 JSONL 损坏。
...
| 迁移后使用了“错误的 Key”或 provider | 认证属于目标 `CODEX_HOME`，但 task 仍保留源 provider 名称 | 独立验证目标认证；不要复制 `auth.json`；用 `--destination-model-provider` 对齐结构化 provider |
