...
python3 codex-mover/linux-session-mover/scripts/migrate_codex_session.py \
  ...
  --destination-model-provider "<exact-target-provider-name>"
```

目标 provider 内部名称与源端完全相同时可省略最后一个参数。完整的停机、apply、目标 app-server 重启、逻辑/物理 rollout 路径和恢复说明见 [Linux Codex 单任务迁移](./codex-mover/linux-session-mover/)。
